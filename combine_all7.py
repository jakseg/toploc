...
This script now times batched searches. Run it like:
    ...
    python3 combine_all7.py snowflake ivf --sweep
...
if "H_CACHED" in os.environ:
    ...
    H_GRID = [int(x) for x in os.environ.get("H_GRID", "512,1024,4096,8192").split(",")]
NP_SINGLE = int(os.environ.get("NP", 128))
ALPHA = float(
    os.environ.get("ALPHA", 0.1)
)  # TopLoc+ drift threshold (single value fallback)
...
def toploc_assign(q_emb, cached_ids, cached_vecs, nprobe, use_ip):
    ...
    actual_nprobe = min(nprobe, len(cached_ids))
...
if SWEEP:
    ...
    out_csv = (
        f"results/raw/ivf/" f"ivf_{model_name}_Hgrid_Agrid_mmap{int(USE_MMAP)}.csv"
    )
...
if rows:
    ...
    best_p = max(rows, key=lambda r: (r["toplocplus_NDCG@10"], -r["toplocplus_fu_ms"]))
    print(
