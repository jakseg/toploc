...
for conv_id, turns in conversations.items():
    ...
    cached_centroid_ids = c0_indices[0].astype("int64")
    ...
    if ivf_index.metric_type == faiss.METRIC_INNER_PRODUCT:
        temp_flat_idx = faiss.IndexFlatIP(ivf_index.d)
    else:
        temp_flat_idx = faiss.IndexFlatL2(ivf_index.d)
    temp_flat_idx.add(cvecs)
    conv_cache_flat[conv_id] = temp_flat_idx  # Save it for follow-ups!
    ...
    # 1. Score the cached centroids (BLAS-optimized and super fast)
    coarse_dists, local_labels = flat_idx.search(fu_embs, NP)

    # 2. Map local labels (0 to H-1) back to the actual global centroid IDs
...
def timed_sweep():
    ...
    for conv_id, turns in conversations.items():
        ...
        # Time the follow-up batch using the prebuilt flat index
