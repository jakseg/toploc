...
# Optional: restrict evaluation to a comma-separated set of conversation IDs,
# e.g. CONV_IDS="31,32,33". Leave unset to evaluate everything. Useful if your
# topics file mixes CAsT 2019 and 2020 and you want to match a single column.
_conv_filter = os.environ.get("CONV_IDS", "").strip()
CONV_FILTER = set(c.strip() for c in _conv_filter.split(",") if c.strip()) or None
...
def turn_sort_key(turn_key):
    """Sort turns by (conv number, turn number) numerically, so e.g. 32_10
    comes AFTER 32_2 (string sorting would get this wrong)."""
    parts = turn_key.split("_")
    try:
        return (int(parts[0]), int(parts[1]))
    except (ValueError, IndexError):
        return (parts[0], turn_key)
...
# Order the turns within each conversation by their real turn number.
for conv_id in conversations:
    conversations[conv_id].sort(key=turn_sort_key)

if CONV_FILTER:
    conversations = {c: t for c, t in conversations.items() if c in CONV_FILTER}
    print(f"Filtered to {len(conversations)} conversations: {sorted(CONV_FILTER)}")
...
times, ndcgs, mrrs = [], [], []
ran_turns = 0  # turns actually executed (for warmup bookkeeping)
scored_turns = 0  # turns that had qrels and contributed to NDCG/MRR

for conv_id, turns in conversations.items():
    q0_key = turns[0]

    # ---- TURN 0: ALWAYS run it to build the cache + warm up. ----
    # We score it only if it happens to be a judged turn. Crucially, we do NOT
    # skip the whole conversation when turn 0 is unjudged -- later turns may be
    # judged and they need this cache.
    ...
    # ---- TURNS 1+: search within the cached centroids; score if judged. ----
...
print("=" * 60)
print(f"Turns run (cache+timing):  {ran_turns}")
print(f"Turns scored (had qrels):  {scored_turns}")
